...
結局の原因は全部 **Imagerの設定が正しく書き込まれていなかったこと** でした。

やらかしポイントが2つ：

1. **ユーザー名に使えない文字を入れていた**
   - Imagerで `The username must be lowercase and contain only letters numbers underscore and hyphens` というエラーが出ていた
   - ユーザー名の設定が保存されず、ユーザーが存在しない状態でSSHが動いていた

2. **「カスタマイズ設定を適用しますか？」で「いいえ」を押していた（もしくは気づいていなかった）**
   - 書き込み完了の最後に出るダイアログで「はい」を押さないと設定が全部無視される

### 解決した手順

Imagerで焼き直すとき、この順番と確認が大事でした：
...
起動まで2〜3分待ってから接続。これで繋がりました。

### 途中でSSHが有効化されていない場合の応急処置

焼き直し不要でSSHだけ有効にする方法もあります。SDカードをPCに挿して、`bootfs` ドライブの直下に `ssh` という名前の空ファイルを置くだけです。
